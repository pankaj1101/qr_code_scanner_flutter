...
// Method to toggle flash
  Future<void> toggleFlash() async {
    if (cameraController == null || !cameraController!.value.isInitialized) {
      return; // Ensure the camera is initialized
    }

    final currentFlashMode = cameraController!.value.flashMode;

    // Determine the next flash mode
    ...
    try {
      await cameraController
          ?.setFlashMode(nextFlashMode); // Set the new flash mode
      state.value =
          state.value.copyWith(flashMode: nextFlashMode); // Update state
    } catch (e) {
